...
static ext4_fsblk_t get_sb_block(void **data)
{
	...
	if (*options && *options != ',') {
		printk(KERN_ERR "SplitFS: Invalid sb specification: %s\n",
		       (char *) *data);
		return 1;
	}
	if (*options == ',')
			    ...
			    int is_remount)
{
	...
	if (m->flags & MOPT_NOSUPPORT) {
		...
		if (arg && (arg > (1 << 30) || !is_power_of_2(arg))) {
			ext4_msg(sb, KERN_ERR,
				 "SplitFS: inode_readahead_blks must be "
...
static int ext4_run_lazyinit_thread(void)
{
	...
	if (IS_ERR(ext4_lazyinit_task)) {
		...
		kfree(ext4_li_info);
		ext4_li_info = NULL;
		printk(KERN_CRIT "SplitFS: error %d creating inode table "
...
static int ext4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (test_opt(sb, DATA_FLAGS) == EXT4_MOUNT_JOURNAL_DATA) {
		printk_once(KERN_WARNING "SplitFS: Warning: mounting with data=journal disables delayed allocation, dioread_nolock, O_DIRECT and fast_commit support!\n");
...
no_journal:
	...
	if (!EXT4_SB(sb)->rsv_conversion_wq) {
		printk(KERN_ERR "SplitFS-fs: failed to create workqueue\n");
		ret = -ENOMEM;
		goto failed_mount4;
	}
...
static inline void register_as_ext2(void)
{
	...
	if (err)
		printk(KERN_WARNING
		       "SplitFS-fs: Unable to register as ext2 (%d)\n", err);
}
...
static inline void register_as_ext3(void)
{
	...
	if (err)
		printk(KERN_WARNING
		       "SplitFS-fs: Unable to register as ext3 (%d)\n", err);
}
