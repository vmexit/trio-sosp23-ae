...
void ext4_ext_init(struct super_block *sb)
{
	...
	if (ext4_has_feature_extents(sb)) {
		...
		printk(KERN_INFO "SplitFS: file extents enabled"
#ifdef AGGRESSIVE_TEST
		       ", aggressive tests"
#endif
#ifdef CHECK_BINSEARCH
...
void ext4_ext_release(struct super_block *sb)
{
	...
	if (EXT4_SB(sb)->s_ext_blocks && EXT4_SB(sb)->s_ext_extents) {
		...
		printk(KERN_ERR "SplitFS-fs: %lu blocks in %lu extents (%lu ave)\n",
			sbi->s_ext_blocks, sbi->s_ext_extents,
			sbi->s_ext_blocks / sbi->s_ext_extents);
		printk(KERN_ERR "SplitFS-fs: extents: %lu min, %lu max, max depth %lu\n",
...
static int ext4_alloc_file_blocks_impl(handle_t *handle, struct file *file, 
									    ext4_lblk_t offset, ext4_lblk_t len, 
										loff_t new_size, int flags, int relink)
{
			...
			loff_t offset, loff_t len, int relink)
{
	...
	if (mode & FALLOC_FL_PUNCH_HOLE) {
		...
		if (relink) {
			ret = ext4_punch_hole_impl(handle, inode, offset, len, relink);
		}
		else {
