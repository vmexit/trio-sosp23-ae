			      ...
			      struct ext4_new_group_data *input)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "SplitFS: adding %s group %u: %u blocks "
		       "(%d free, %u reserved)\n",
...
next_group:
	...
	if (test_opt(sb, DEBUG)) {
		...
		printk(KERN_DEBUG "SplitFS: adding a flex group with "
		       ...
		       ext4_group_t group)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG
		       "SplitFS: ext4_add_new_gdb: adding group block %lu\n",
		       gdb_num);
			     ...
			     struct ext4_new_flex_group_data *flex_gd)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "SplitFS: added group %u:"
...
errout:
	...
	if (!err) {
		if (test_opt(sb, DEBUG))
			printk(KERN_DEBUG "SplitFS: extended group to %llu "
			       "blocks\n", ext4_blocks_count(es));
