...
class PosixMmapReadableFile final : public RandomAccessFile {
 public:
	PosixMmapReadableFile(std::string filename, int fd)
	      : fd_(fd), filename_(std::move(filename)) {}

    ~PosixMmapReadableFile() override { close(fd_); }
    		...
    		char* scratch) const override
    {
      Status status;
      while (true) {
        ::ssize_t read_size = ::pread(fd_, scratch, n, offset);
        if (read_size < 0) {  // Read error.
          if (errno == EINTR) {
            continue;  // Retry
          }
...
class PosixEnv : public Env {
 public:
  ...
  Status GetChildren(const std::string& directory_path,
    ...
    result->clear();
    ::DIR* dir = sufs_opendir(directory_path.c_str());
    if (dir == nullptr) {
      return PosixError(directory_path, errno);
    }
    struct ::dirent* entry;
    while ((entry = sufs_glibc_readdir(dir)) != nullptr) {
