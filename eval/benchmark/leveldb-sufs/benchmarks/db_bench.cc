...
class Stats {
 ...
 public:
  ...
  void Report(const Slice& name) {
    ...
    std::fprintf(stdout, "%-12s : %11.3f ops/s;%s%s\n",
                 name.ToString().c_str(), done_ / (seconds_ * 1.0),
...
class Benchmark {
 ...
 private:
  ...
  void WriteSeq(ThreadState* thread) { DoWrite(thread, true, true); }

  void WriteRandom(ThreadState* thread) { DoWrite(thread, false, true); }

  void DoWrite(ThreadState* thread, bool seq, bool stat) {
    if (num_ != FLAGS_num) {
      char msg[100];
