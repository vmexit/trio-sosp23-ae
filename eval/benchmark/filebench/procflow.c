...
static void * worker_mode_wrapper(void * arg)
{
    int ret;
    struct fbparams * fbparams = arg;
...
static int procflow_createproc(procflow_t * procflow)
{
    struct fbparams * fbparams = calloc(1, sizeof(struct fbparams));

    fbparams->procname = procflow->pf_name;
    fbparams->instance = procflow->pf_instance;

    procflow->pf_running = 0;

    return pthread_create(&(procflow->pf_tid), NULL, worker_mode_wrapper, fbparams);
...
procflow_createnwait(void *unused)
{
    ...
    while (procflow != NULL) {
        ...
        if (pthread_join(procflow->pf_tid, NULL) < 0)
            pthread_exit(0);
