...
flowoplib_createfile(threadflow_t *threadflow, flowop_t *flowop)
{
    ...
    if ((ret = flowoplib_iobufsetup(threadflow, flowop, &iobuf, iosize)) != FILEBENCH_OK)
    {
        return (ret);
    }

	flowop_beginop(threadflow, flowop);
	err = fileset_createfile(flowop->fo_fileset,
		file, openflag, 0666, flowoplib_fileattrs(flowop), iobuf, iosize);
	flowop_endop(threadflow, flowop, 0);
    ...
    fileset_unbusy(file, TRUE, TRUE, 0);
...
flowoplib_readwholefile(threadflow_t *threadflow, flowop_t *flowop)
{
    ...
    fileset_t  * fileset = flowop->fo_fileset;

    if (avd_get_bool(flowop->fo_rotatefd))
        tid = threadflow->tf_utid;

    if ((ret = flowoplib_pickfile(&file, flowop,
        FILESET_PICKEXISTS, tid)) != FILEBENCH_OK) {
        filebench_log(LOG_DEBUG_SCRIPT,
            "flowop %s failed to pick file", flowop->fo_name);
        return (ret);
	...
	if ((ret = FB_GET(path, iobuf, iosize)) > 0)
		bytes += ret;
...
flowoplib_writewholefile(threadflow_t *threadflow, flowop_t *flowop)
{
    ...
    if ((ret = FB_SET(path, iobuf, iosize)) > 0)
        bytes += ret;
