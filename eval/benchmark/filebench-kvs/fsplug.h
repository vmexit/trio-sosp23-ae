...
typedef struct fsplug_func_s {
    ...
    int (*fsp_get)(char *, caddr_t, fbint_t);
    int (*fsp_set)(char *, caddr_t, fbint_t);
} fsplug_func_t;
...
#define FB_GET(path, iobuf, iosize) \
    (*fs_functions_vec->fsp_get)(path, iobuf, iosize)

#define FB_SET(path, iobuf, iosize) \
    (*fs_functions_vec->fsp_set)(path, iobuf, iosize)
