...
static int fb_lfs_get(char *, caddr_t, fbint_t);
static int fb_lfs_set(char *, caddr_t, fbint_t);
...
static int
fb_lfs_get(char * path, caddr_t iobuf, fbint_t iosize)
{
#if 0
    printf("path is %s, iobuf is %lx, iosize is %d\n", path, iobuf, iosize);
#endif

    return sufs_get(path, iobuf, iosize);
}

static int
fb_lfs_set(char * path, caddr_t iobuf, fbint_t iosize)
{
    return sufs_set(path, iobuf, iosize);
}
